...
namespace Game.Server
{
    ...
    public class ServerCommandStorageSystem : Singleton<ServerCommandStorageSystem>, IServerSystem
    {
        ...
        /// TODO: 大的话考虑分包
        /// </summary>
        public void SyncAllCommands(ClientSession conn)
        {
            ...
            msg.SyncedBattleServerTick = syncFrameMax;
            msg.CommandsSet.AddRange(lst);
            NetworkServer.Instance.SendMessageToClient(conn.SessionId, EProtoType.CommandAllRsp, msg);

            if (lst.Count > 0)
            {
                GameHelper_Common.LogDebug($"Server: SyncCommands_All at {battleServerTick} frames:0->{syncFrameMax} {frames.GetString()}");
            }
